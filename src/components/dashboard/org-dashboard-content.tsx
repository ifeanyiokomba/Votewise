...
export function OrgDashboardContent({ me }: { me: MeResponse }) {
  ...
  return (
    <div className="space-y-8 p-4 sm:p-6 lg:p-8">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          <StatCard label="Active Elections" value={formatNumber(stats.orgStats.activeElections)} icon={Vote} hint="LIVE + scheduled" />
          <StatCard label="Total Voters" value={formatNumber(stats.orgStats.voters)} icon={Users} hint="Across all elections" />
          <StatCard label="Candidates" value={formatNumber(stats.orgStats.candidates)} icon={UserSquare2} />
          <StatCard label="Votes Cast" value={formatNumber(stats.orgStats.totalVotes)} icon={ClipboardCheck} hint="All-time" />
          <StatCard label="Pending Tickets" value={formatNumber(stats.orgStats.pendingTickets)} icon={LifeBuoy} hint="Open support tickets" />
          <StatCard label="Security Alerts" value={formatNumber(stats.stats.unresolved)} icon={ShieldAlert} hint={`${stats.stats.critical} critical`}
            trend={stats.stats.unresolved > 0 ? { value: "needs attention", positive: false } : { value: "all clear", positive: true }} />
        </div>
      ) : null}
      ...
      {!loading && !error && (
        <Card>
          ...
          <CardContent className="grid gap-2 pt-4 sm:grid-cols-2 lg:grid-cols-4">
            <QuickAction href="/dashboard/elections" icon={Plus} title="Create election" description="Start a new vote" />
            <QuickAction href="/dashboard/elections" icon={Upload} title="Import voters" description="Upload CSV / XLSX" />
            <QuickAction href="/dashboard/elections" icon={BarChart3} title="View results" description="See live tallies" />
            <QuickAction href="/dashboard/support" icon={LifeBuoy} title="Contact support" description="We respond fast" />
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <section className="lg:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            ...
            <Button asChild variant="ghost" size="sm">
              <Link href="/dashboard/elections">View all <ArrowRight className="h-3.5 w-3.5" /></Link>
            </Button>
          </div>
          {loading ? (
            ...
            <EmptyState title="No active elections" description="Create an election to get it ready for voting." icon={Vote}
              action={<Button asChild><Link href="/dashboard/elections"><Plus className="h-4 w-4" /> Create election</Link></Button>} />
          ) : (
            <div className="grid gap-4 sm:grid-cols-2">
              {elections.slice(0, 4).map((el, i) => (<ElectionCard key={el.id} election={el} index={i} />))}
            </div>
          )}
        </section>
        ...
        <aside className="space-y-6">
          <Card>
            <CardHeader className="border-b pb-4">
              <CardTitle className="flex items-center gap-2 text-base"><Activity className="h-4 w-4 text-primary" />Election health</CardTitle>
              <CardDescription>Turnout across active elections.</CardDescription>
            </CardHeader>
            <CardContent className="pt-4">
              {activeElections.length === 0 ? (
                <p className="py-6 text-center text-xs text-muted-foreground">No active elections to monitor yet.</p>
              ) : (
                      ...
                      return (
                        <li key={el.id} className="space-y-1.5">
                          <div className="flex items-center justify-between gap-2">
                            <Link href={`/dashboard/elections/${el.id}`} className="min-w-0 flex-1 truncate text-sm font-medium hover:underline">{el.name}</Link>
                          ...
                          <p className="text-[10px] text-muted-foreground">{formatNumber(votes)} of {formatNumber(voters)} voters</p>
                        </li>
                      );
                    })}
                  </ul>
          ...
          {me?.organization?.id && <LiveActivityFeed organizationId={me.organization.id} />}

          <Card>
            <CardHeader className="border-b pb-4">
              <CardTitle className="flex items-center gap-2 text-base"><ScrollText className="h-4 w-4 text-primary" />Recent audit</CardTitle>
                    ...
                    {recentAudit.slice(0, 6).map((log) => (
                      <li key={log.id} className="flex gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xs font-medium">{AUDIT_ACTION_LABELS[log.action] ?? log.action.replace(/_/g, " ").toLowerCase()}</p>
                          <p className="text-[10px] text-muted-foreground">{log.resource}{log.result && log.result !== "SUCCESS" ? ` · ${log.result.toLowerCase()}` : ""} · {formatRelative(log.timestamp)}</p>
                        </div>
                      </li>
                    ))}
                  </ol>
    ...
    <TooltipProvider delayDuration={300}>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link href={href} className={cn("group flex items-center gap-3 rounded-lg border bg-card p-3 transition-all hover:border-primary/40 hover:bg-accent")}>
